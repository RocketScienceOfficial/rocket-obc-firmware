...
static UBloxSamM10QConfig s_UBloxSamM10QConfig;
static UBloxSamM10QData s_UBloxSamM10QData;
...
VOID initSensors(VOID)
{
    ...
    DRIVER_CALL(uBloxSamM10QInitI2C(&s_UBloxSamM10QConfig, I2C, SDA, SCL));
}
...
BOOL checkGPS(VOID)
{
    DRIVER_CALL(uBloxSamM10QReadData(&s_UBloxSamM10QConfig, &s_UBloxSamM10QData));

    if (s_UBloxSamM10QData.isFinishedSentence)
    {
        if (nmeaCheckSentence(s_UBloxSamM10QData.sentence))
        {
            NMEASentence s = nmeaGetSentenceId(s_UBloxSamM10QData.sentence);

            if (s == NMEA_SENTENCE_GGA)
            {
                NMEASentence_GGA frame = {0};
                nmeaParse_GGA(s_UBloxSamM10QData.sentence, &frame);
            ...
            else if (s == NMEA_SENTENCE_GLL)
            {
                ...
                nmeaParse_GLL(s_UBloxSamM10QData.sentence, &frame);
            ...
            else if (s == NMEA_SENTENCE_GNS)
            {
                ...
                nmeaParse_GNS(s_UBloxSamM10QData.sentence, &frame);
