...
void loop()
{
    if (runEvery(1000, &s_TimerOffset))
    {
        ...
        double accel_x = kalman(&s_KalmanData, mpu6050Data.accel_x);
        double accel_y = kalman(&s_KalmanData, mpu6050Data.accel_y);
        double accel_z = kalman(&s_KalmanData, mpu6050Data.accel_z);
        double rot_x = kalman(&s_KalmanData, mpu6050Data.rot_x);
        double rot_y = kalman(&s_KalmanData, mpu6050Data.rot_y);
        double rot_z = kalman(&s_KalmanData, mpu6050Data.rot_z);
