...
static LoRaData *s_LoraDatas[LORA_MAX_INSTANCES];
...
static LoRaData *__loraDataFromGPIO(uint gpio)
{
    for (size_t i = 0; i < s_LoraDatasCount; i++)
    {
        LoRaData *d = s_LoraDatas[i];
...
void __loraOnDio0Rise(uint gpio, uint32_t events)
{
    ...
    LoRaData *data = __loraDataFromGPIO(gpio);
