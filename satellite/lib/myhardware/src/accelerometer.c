...
static i2c_inst_t *getI2C(AccelerometerConfig *config)
{
...
static void mpu6050Reset(AccelerometerConfig *config, int lpFilter, int accelSensLevel, int gyroSensLevel)
{
...
static void mpu6050ReadRawValues(AccelerometerConfig *config, int16_t accel[3], int16_t gyro[3], int16_t *temp)
{
    uint8_t buffer[6];
...
FUNCRESULT accelerometerInit(AccelerometerConfig *config, int i2c, int sda, int scl, int lpFilter, int accelSensLevel, int gyroSensLevel)
{
    ...
    memset(config, 0, sizeof(AccelerometerConfig));
    ...
    if (FUNCFAILED(accelerometerCheck(config, &result)))
    {
...
static float convertAcceleration(AccelerometerConfig *config, int16_t raw, int16_t offset, float scale)
{
...
static float convertGyro(AccelerometerConfig *config, int16_t raw, float scale)
{
