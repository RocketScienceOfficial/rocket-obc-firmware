...
static dataman_frame_t _get_frame(void)
{
    ...
    dataman_frame_t frame = {
        ...
        .dt_us = dt,
        .accRaw = sensors_get_frame()->acc1,
        .gyroRaw = sensors_get_frame()->gyro1,
        .magRaw = sensors_get_frame()->mag1,
        .gpsPos = sensors_get_frame()->pos,
        .gpsData = _get_gps_data(),
        .pressure = sensors_get_frame()->press,
        .accNED = ahrs_get_data()->acceleration,
        .velNED = ahrs_get_data()->velocity,
        .posNED = ahrs_get_data()->position,
        .qNED = ahrs_get_data()->orientation,
...
static bool _print_saved_frame(const dataman_frame_t *frame)
{
    if (_validate_frame(frame))
    {
        datalink_frame_data_saved_chunk_t payload = {
            .dt = frame->dt_us,
            .accRawX = frame->accRaw.x,
            .accRawY = frame->accRaw.y,
            .accRawZ = frame->accRaw.z,
            .gyroRawX = frame->gyroRaw.x,
            .gyroRawY = frame->gyroRaw.y,
            .gyroRawZ = frame->gyroRaw.z,
            .magRawX = frame->magRaw.x,
            .magRawY = frame->magRaw.y,
            .magRawZ = frame->magRaw.z,
