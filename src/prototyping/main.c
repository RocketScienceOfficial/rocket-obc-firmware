...
//         {
//             // total += ads786x_read(&ads786xConfig);
//             // hal_spi_cs_select(PIN_CS_ADS);
//             gpio_put(PIN_CS_ADS, 0);

//             uint8_t buffer[2];
//             // hal_spi_read(OBC_SPI, 0, buffer, sizeof(buffer));
//             spi_read_blocking(spi0, 0, buffer, 2);

//             gpio_put(PIN_CS_ADS, 1);

//             // hal_spi_cs_deselect(PIN_CS_ADS);

//             uint16_t data = ((buffer[0] << 8) | buffer[1]);
//             float result = ADC_VREF * (float)data / (1 << 12);
//             total += result;
...
//         //             for (size_t i = 0; i < 100; i++)
//         //             {
//         //                 float data = ads786x_read(&ads786xConfig) * 11;
//         //                 data = data - (-0.054f * data + 0.6f);
//         //                 total += data;

//         // //                hal_serial_printf("POMIAR #%d   %f\n", i + 1, data);
//         //             }

//         //             hal_serial_printf("WYNIK     %f\n", total);
//         //             hal_time_sleep_ms(10);
//         //         }
//     }
...
int main()
{
    ...
    while (true)
    {
        ...
        for (size_t i = 0; i < 100; i++)
        {
            ...
            uint16_t data = ((buffer[0] << 8) | buffer[1]);
            float result = 11 * 2.5f * (float)data / (1 << 12);
            total += result;
